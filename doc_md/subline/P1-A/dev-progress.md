...
- 已完成上线后视觉打磨轮（2026-05-27，逐项 preview 实测）：缩放适配（移动端导航不再隐藏 + 可横滚、cab 状态栏按需换行、跨 320–2560 及横屏无溢出）；Hero 标题中/日文降为 `clamp(44px,4.6vw,88px)` 解决窄栏重叠；HUD 改造（EX-SCORE/COMBO 统一英文、删判定 PGREAT、GROOVE GAUGE 改 IIDX 分段条 + 大号百分比随 combo 填充）；删两页顶部 cab 状态栏与 brand 日文 kana、收紧移动端 header、修复删 kana 引发的 brand 大字/小字重合；GROOVE GAUGE 血条随信号色同步；Hi-Speed/Playfield 控件移到 playfield slate 同行右侧（tweaks 仅留信号色）；Hi-Speed 改 5 档 ×0.6–×1.4；「规格」三语统一改为「特性」；所有大字标题去结尾句号（三语）；资源版本 `v=20260526-11`
