...
- 已将 `AGENTS.md` 升级为唯一持久协作入口，完整承接原仓库结构、部署说明、当前状态、硬约束与同步纪律；旧的平行入口已移除，工作区引用已统一
- 已补充 `.gitattributes` LF 换行基线，明确静态技术边界、公开口径、文档同步和生产部署授权要求
- 完成前端子仓库 `git init`
- 新增前端 `.gitignore`
...
- 已完成上线后视觉打磨轮（2026-05-27，逐项经 preview 实测）：
  ...
  - 特性清单 cap.05「离线优先」→「便携安装」（绿灯·已实现）；新增 cap.06「游戏社区」（IR/谱面库/论坛，红灯·开发中）
  - 资源版本迭代至 `v=20260526-15`（破缓存）
...
- 下载流现状（2026-06-03 复核）：首页 `#download` 已为两枚**静态跳转 tile**（GitHub Releases + QQ 群 650530995），**`download.js` 自动拉取 release 与安装步骤板块已删除**；上文涉及 `download.js` / GitHub 403 退回的验证记录属当时历史、现已不适用（"下载不经 Web 后端"的结论不变）
